...
public abstract class AbstractHashMap<K,V> extends AbstractMap<K,V> {
  ...
  @Override
  public V get(K key) { 
	  return bucketGet(hashValue1(key), hashValue2(key), key); 
  }
   ...
   */
  @Override
  public V remove(K key) { return bucketRemove(hashValue1(key), hashValue2(key), key); }
  ...
  public V put(K key, V value) {
    V answer = bucketPut(hashValue1(key), hashValue2(key), key, value);
  ...
  // For our project need to change hashcode for phrases to sum of chars in phrase
  private int hashValue1(K key) {
    return (int) ((Math.abs(key.hashCode() % capacity)));
  }

  /** Hash function applying secondary MAD method to default hash code. */
  // Change to take in sum of chars
  private int hashValue2(K key) {
	  return (int) (Math.abs(key.hashCode() * 3) % capacity);
  }
   ...
   */
  protected abstract V bucketGet(int h1, int h2, K k);
   ...
   */
  protected abstract V bucketPut(int h1,int h2, K k, V v);
   ...
   * @return   previous value associated with k (or null, if no such entry)
   */
  protected abstract V bucketRemove(int h1, int h2, K k);
}
